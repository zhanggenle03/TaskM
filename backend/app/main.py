...
async def lifespan(app: FastAPI):
    ...
    # ── 启动迁移：补齐 checkin_projects.days 列（多项目按项目自填天数） ──
    try:
        from .database import _ensure_checkin_project_days_column
        _ensure_checkin_project_days_column()
    except Exception as e:
        print(f"[lifespan] checkin_projects.days 迁移失败: {e}", flush=True)
...
_ensure_salary_record_type_column()
# 去掉 salary_records 的 UNIQUE(period, record_type)：允许同月多条工资/奖金记录
_drop_salary_period_unique()
_fix_salary_child_foreign_keys(engine)
ensure_file_edit_lock_columns(engine)
